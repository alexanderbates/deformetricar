...
Deformetrica (\>= 4.3), meeting current R package standards.
...
- [`deformetrica_register_multi()`](https://alexanderbates.github.io/deformetricar/reference/deformetrica_register_multi.md):
  ...
  regulariser. It now accepts a per-object `data_sigma` (scalar, named
  or positional vector) so objects can be weighted differently in one
  fit — e.g. strong homologous neuropils over a weak outer hull.
