...
`apply.mirror.affine()`, `symmetrisel1()`, `otherside()` for the symmetrising map,
and `findcognate()` / `checkL1cognates()` for the left-right cognate search — each
...
`otherside()`, then NBLAST it against the left neurons — the top hit is its putative left
...
**`checkL1cognates()`** — the old interactive rgl inspector, as a static `nat.ggplot`
...
`findcognate()` scales straight to that: pass the whole-CNS dotprops as `db_dps`.
...
- **The five functions are the point.** `apply.mirror.affine()`, `symmetrisel1()`,
  `otherside()`, `findcognate()` and `checkL1cognates()` are just closures over `pre`,
