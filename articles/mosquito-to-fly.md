...
![The mosquito neuropils (coloured by homology) first settling from
their native proportions into the initialised pose (the affine), then
warping onto their fly counterparts (the diffeomorphism) — each matched
fly neuropil drawn in transparent greyscale, inside the fly brain hull
as a translucent light-pink envelope, which stays fixed
throughout.](../reference/figures/mosquito_to_fly.gif)

The mosquito neuropils (coloured by homology) first settling from their
native proportions into the initialised pose (the affine), then warping
onto their fly counterparts (the diffeomorphism) — each matched fly
neuropil drawn in transparent greyscale, inside the fly brain hull as a
translucent light-pink envelope, which stays fixed throughout.
...
sigma[grep("^(ME|LO|LOP)_", names(sigma))] <- 1.5       # optic lobes: pull harder so all three hit their fly target
...
## 5. Animate: first the affine, then the warp

We animate the registration in **two acts** so the cross-species story
reads. **Act 1** shows the *affine* — each structure appears in its
native mosquito proportions and settles into the initialised pose (the
...
# its native mosquito proportions, then settling into the initialised pose. We apply that
# linear map CENTRED on the view (dropping the translation) so the fly and the camera
# stay put, then prepend this reveal to the warp.
Tp     <- function(P) Morpho::applyTransform(pre$apply(P), M)      # native pts -> aligned pts
o      <- Tp(matrix(0, 1, 3))
Lmat   <- rbind(Tp(matrix(c(1,0,0),1))-o, Tp(matrix(c(0,1,0),1))-o, Tp(matrix(c(0,0,1),1))-o)
Linv   <- solve(Lmat)
reveal <- function(keys, centre_keys, nf = 9) {
  cc <- colMeans(do.call(rbind, lapply(srcs[centre_keys], xyzmatrix)))
  setNames(lapply(keys, function(k) { av <- xyzmatrix(srcs[[k]])
    pv <- sweep(sweep(av, 2, cc) %*% t(Linv), 2, cc, "+")          # centred native proportions
    lapply(seq(0, 1, length.out = nf), function(a) { m <- srcs[[k]]; xyzmatrix(m) <- (1-a)*pv + a*av; m })
  }), keys)
}
a1     <- reveal(c("outer", show), show)                           # reveal centred on all neuropils
staged <- setNames(lapply(c("outer", show), function(k) c(a1[[k]], flows[[k]])), c("outer", show))
# Colour by functional group (L/R matched), shaded within group, LaCroix-inspired:
# optic = blue, mushroom body = pink-purple, central complex = yellow-orange, the SEZ
# AMMC green, and the rest of the mid brain cerise.
grp <- function(b) {
  if (b %in% c("ME","LO","LOP")) "optic"
  else if (b %in% c("AMMC")) "sez"
  else if (b %in% c("CA","PED","VL","ML")) "mb"
  else if (b %in% c("FB","EB","PB","NO")) "cx"
  else "midbrain"
}
grp_ramp <- list(optic=c("#1BA3C6","#B3ECF7"), sez=c("#2CB11B","#CBEFA0"),
                 mb=c("#C1478E","#EDBFE0"), cx=c("#F5850B","#FFD24D"),
                 midbrain=c("#C1275E","#F2A6C2"))
sb  <- sub("_[LR]$", "", show); groups <- vapply(sb, grp, character(1))
cols <- setNames(character(length(show)), show)
for (g in names(grp_ramp)) { gb <- unique(sb[groups == g]); if (!length(gb)) next
  ramp <- setNames(grDevices::colorRampPalette(grp_ramp[[g]])(length(gb)), gb)
  cols[groups == g] <- ramp[sb[groups == g]] }
...
# is upside down.
rmat <- diag(c(1, -1, 1, 1))
# The aedes hull is drawn first (behind), baby-pink + translucent so we watch it deform
# behind the coloured neuropils; a slow delay keeps the whole GIF ~5 s so the warp reads.
cols_all  <- c(list(outer = "#F7C6D0"), as.list(cols))
alpha_all <- c(outer = 0.08, setNames(rep(0.9, length(show)), show))
ggplot_flow_gif(staged[c("outer", show)], cols = cols_all, alpha = alpha_all,
                rotation_matrix = rmat, delay = 0.16,
...
a1cx      <- reveal(cxn, cxn)                      # same two acts, reveal centred on the CX
staged_cx <- setNames(lapply(cxn, function(k) c(a1cx[[k]], flows[[k]])), cxn)
ggplot_flow_gif(staged_cx[cxn], cols = cols[cxn], rotation_matrix = rmat, delay = 0.16,
